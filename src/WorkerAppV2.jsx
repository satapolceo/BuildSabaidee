...
function WorkerAppV2({ onNavigate, t, language = 'TH', workersList = [], projectsList = [] }) {
  ...
  useEffect(() => {
    const syncTasks = getWorkerTasks(loadFromStorage(WORKER_STORAGE_KEYS.tasks, []), currentWorker.id, siteName);
    setTasks(syncTasks);
  ...
  const latestPhotoReports = useMemo(
    () => photoReports.filter((entry) => entry.workerId === currentWorker.id).slice(-3).reverse(),
    [photoReports, currentWorker.id]
  );
  const latestIssues = useMemo(
    () => issues.filter((entry) => entry.workerId === currentWorker.id).slice(-3).reverse(),
    [issues, currentWorker.id]
  );
  const latestRequests = useMemo(
    () => materialRequests.filter((entry) => entry.workerId === currentWorker.id).slice(-3).reverse(),
  ...
  const latestNotifications = [
    { id: 'attendance', title: pickText(t, statusKey, 'Attendance updated') },
  ...
  const submitPhoto = () => {
    ...
    pushToast('worker_report_success', 'Work photo submitted');
  ...
  const submitIssue = (forceCritical = false) => {
    ...
    pushToast('worker_sos_success', forceCritical ? 'SOS sent' : 'Issue sent');
    setActiveTab(TAB_HOME);
  };
  ...
  const submitRequest = () => {
    ...
    pushToast('worker_request_success', 'Material request sent');
    setActiveTab(TAB_HOME);
  };
  ...
  const navItems = [
    ...
    { id: TAB_SUBMIT, label: pickText(t, 'worker_nav_submit', 'Submit'), icon: Send },
            ...
            <div className="space-y-4">
              <section className="overflow-hidden rounded-[1.9rem] bg-white shadow-[0_18px_40px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/80">
                <div className="bg-gradient-to-br from-slate-950 via-blue-900 to-blue-700 p-5 text-white">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-2 text-2xl font-semibold">{pickText(t, statusKey, 'Attendance')}</div>
                    </div>
                ...
                <div className="space-y-3 p-4">
                  <input value={attendanceNote} onChange={(event) => setAttendanceNote(event.target.value)} placeholder={pickText(t, 'worker_note_label', 'Note')} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-blue-500" />
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => handleAttendance('checkin')} className="rounded-[1.3rem] bg-blue-700 px-4 py-4 text-sm font-semibold text-white active:scale-[0.99]">{pickText(t, 'worker_checkin_cta', 'Check In')}</button>
                    <button onClick={() => handleAttendance('checkout')} className="rounded-[1.3rem] bg-emerald-600 px-4 py-4 text-sm font-semibold text-white active:scale-[0.99]">{pickText(t, 'worker_checkout_cta', 'Check Out')}</button>
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {[
                    { id: 'photo', title: pickText(t, 'worker_photo', 'Upload Photo'), icon: Camera, tone: 'blue', onClick: () => setActiveTab(TAB_SUBMIT) },
                    { id: 'request', title: pickText(t, 'worker_material', 'Request Material'), icon: Package, tone: 'orange', onClick: () => setActiveTab(TAB_SUBMIT) },
                    { id: 'sos', title: pickText(t, 'worker_sos', 'Report Issue / SOS'), icon: AlertTriangle, tone: 'red', onClick: () => { setIssueForm((current) => ({ ...current, urgency: 'critical' })); setActiveTab(TAB_SUBMIT); } },
                  ].map((action) => {
                    const Icon = action.icon;
                    return (
                      <button key={action.id} onClick={action.onClick} className={`flex items-center gap-4 rounded-[1.5rem] border p-4 text-left shadow-sm active:scale-[0.99] ${action.tone === 'red' ? 'border-rose-200 bg-rose-50' : action.tone === 'orange' ? 'border-orange-200 bg-orange-50' : 'border-blue-200 bg-blue-50'}`}>
                        <div className={`flex h-14 w-14 items-center justify-center rounded-2xl text-white ${action.tone === 'red' ? 'bg-rose-600' : action.tone === 'orange' ? 'bg-orange-500' : 'bg-blue-700'}`}><Icon className="h-6 w-6" /></div>
                        <div className="flex-1 text-base font-semibold text-slate-900">{action.title}</div>
              ...
              <section className="rounded-[1.6rem] bg-white p-4 shadow-sm ring-1 ring-slate-200/80">
                <div className="mb-3 text-base font-semibold text-slate-900">{pickText(t, 'worker_today_title', 'Today')}</div>
              ...
              <section className="grid grid-cols-1 gap-4">
                ...
                <div className="rounded-[1.6rem] bg-white p-4 shadow-sm ring-1 ring-slate-200/80">
                  <div className="mb-3 text-base font-semibold text-slate-900">{pickText(t, 'worker_latest_alerts_title', 'Latest Notifications')}</div>
                  <div className="space-y-2">
          ...
          {activeTab === TAB_SUBMIT ? (
            <div className="space-y-4">
              <FormCard title={pickText(t, 'worker_photo', 'Upload Photo')}>
                <input value={photoForm.category} onChange={(event) => setPhotoForm((current) => ({ ...current, category: event.target.value }))} placeholder={pickText(t, 'worker_report_category', 'Job category')} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" />
                <textarea value={photoForm.detail} onChange={(event) => setPhotoForm((current) => ({ ...current, detail: event.target.value }))} placeholder={pickText(t, 'worker_report_details', 'Details')} rows={3} className="mt-3 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" />
                ...
                <HistoryList items={latestPhotoReports} emptyLabel={pickText(t, 'worker_recent_reports', 'Recent reports')} renderItem={(item) => `${item.category} • ${pickText(t, item.status === 'queued' ? 'worker_saved_locally' : 'worker_record_status_submitted', item.status)}`} />
              </FormCard>

              <FormCard title={pickText(t, 'worker_sos', 'Issue / SOS')}>
                <div className="grid grid-cols-2 gap-3">
                ...
                <textarea value={issueForm.detail} onChange={(event) => setIssueForm((current) => ({ ...current, detail: event.target.value }))} placeholder={pickText(t, 'worker_sos_desc', 'Describe the issue')} rows={3} className="mt-3 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" />
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                  ...
                  <button onClick={() => submitIssue(true)} className="rounded-[1.2rem] bg-rose-600 px-4 py-3 text-sm font-semibold text-white">{pickText(t, 'worker_sos', 'SOS')}</button>
                </div>
                <HistoryList items={latestIssues} emptyLabel={pickText(t, 'worker_latest_alerts_title', 'Latest notifications')} renderItem={(item) => `${item.category} • ${item.urgency}`} />
              </FormCard>
              ...
              <FormCard title={pickText(t, 'worker_material', 'Request Material')}>
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                  ...
                  <input value={requestForm.note} onChange={(event) => setRequestForm((current) => ({ ...current, note: event.target.value }))} placeholder={pickText(t, 'worker_note_label', 'Note')} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" />
                </div>
                ...
                <button onClick={submitRequest} className="mt-3 w-full rounded-[1.2rem] bg-orange-500 px-4 py-3 text-sm font-semibold text-white">{pickText(t, 'worker_open_request_short', 'Request items')}</button>
                <HistoryList items={latestRequests} emptyLabel={pickText(t, 'worker_pending_title', 'Pending')} renderItem={(item) => `${item.itemName} • ${item.quantity} ${item.unit}`} />
              </FormCard>
            </div>
          ) : null}
        ...
        <div className="border-t border-slate-200 bg-white px-3 pb-[calc(env(safe-area-inset-bottom,0px)+0.85rem)] pt-3">
          <div className="grid grid-cols-4 gap-2">
            {navItems.map((item) => {
              ...
              return (
                <button key={item.id} onClick={() => setActiveTab(item.id)} className={`rounded-2xl px-2 py-3 text-center ${active ? 'bg-blue-700 text-white' : 'text-slate-500'}`}>
...
function FormCard({ title, children }) {
  return <div className="rounded-[1.6rem] bg-white p-4 shadow-sm ring-1 ring-slate-200/80"><div className="mb-3 text-base font-semibold text-slate-900">{title}</div>{children}</div>;
}

function FilePicker({ imageData, onChange, label, optional = false, t }) {
  return (
    <label className="mt-3 block rounded-[1.3rem] border border-dashed border-slate-300 bg-slate-50 p-4 text-center">
      <input type="file" accept="image/*" capture="environment" onChange={onChange} className="hidden" />
      {imageData ? <img src={imageData} alt={label} className="mx-auto mb-3 max-h-40 rounded-2xl object-cover" /> : <UploadCloud className="mx-auto h-8 w-8 text-slate-400" />}
...
function HistoryList({ items, emptyLabel, renderItem }) {
  return (
    <div className="mt-4 space-y-2">
      {items.length ? items.map((item) => <div key={item.id} className="rounded-xl bg-slate-50 px-3 py-2 text-sm text-slate-700">{renderItem(item)}</div>) : <div className="rounded-xl bg-slate-50 px-3 py-2 text-sm text-slate-500">{emptyLabel}</div>}
    </div>
  );
}

function MetricCard({ label, value }) {
  return <div className="rounded-2xl bg-slate-50 px-4 py-3"><div className="text-xs text-slate-500">{label}</div><div className="mt-1 text-xl font-semibold text-slate-900">{value}</div></div>;
}
